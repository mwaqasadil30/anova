...
import { gray600, gray900 } from 'styles/colours';
...
const StyledTableRow = styled(({ rowOriginalData, isClickable, ...props }) => (
  ...
  ${({ rowOriginalData }: { rowOriginalData: AssetSummaryModel }) => {
    const {
...
const recordsDefault: AssetSummaryModel[] = [];
...
interface Props extends FilterFormProps {
  ...
  apiResponse?: AssetSummaryDto | null;
  pageCount?: number;
  totalRows: number;
  pageSize: number;
  pageNumber: number;
...
const AssetSummaryTable = ({
  ...
  const tableColumnsWithoutWidth = [
    {
      ...
      // @ts-ignore
      accessor: (row: AssetSummaryModel) => {
        const unitType = getValidUnitDisplayType({
          unitDisplayType,
          dataChannelType: row.dataChannelType,
          hasValidDisplayValue: !!row.displayUOMParams?.alarmLevels,
        });
        switch (unitType) {
          case UnitDisplayType.Scaled:
            return row.scaledUOMParams?.alarmLevels;
          case UnitDisplayType.Display:
            return row.displayUOMParams?.alarmLevels;
          case UnitDisplayType.PercentFull:
            return row.percentFullUOMParams?.alarmLevels;
          default:
            return '';
        }
      },
    ...
    },
    {
      ...
      Cell: (props: ReactTableCell<AssetSummaryModel>) => (
        <NameCell
          {...props}
    ...
    },
    {
      ...
      accessor: (row: AssetSummaryModel) => {
        const displayValue = row.displayUOMParams?.deliverable;
        const scaledValue = row.scaledUOMParams?.deliverable;
        const percentFullValue = row.percentFullUOMParams?.deliverable;
        const unitType = getValidUnitDisplayType({
          unitDisplayType,
          dataChannelType: row.dataChannelType,
          hasValidDisplayValue: isNumber(displayValue) && !!row.displayUnits,
        });
        switch (unitType) {
          case UnitDisplayType.Scaled:
            return isNumber(scaledValue) && row.scaledUnits
              ? `${scaledValue} ${row.scaledUnits}`
              : '';
          case UnitDisplayType.Display:
            return isNumber(displayValue) && row.displayUnits
              ? `${displayValue} ${row.displayUnits}`
              : '';
          case UnitDisplayType.PercentFull:
            return isNumber(percentFullValue) ? `${percentFullValue} %` : '';
          default:
            return '';
        }
      },
    ...
    },
    {
      ...
      accessor: (row: AssetSummaryModel) => {
        const percentFullValue = row.percentFullUOMParams?.reading;
        if (isNumber(percentFullValue)) {
          return `${round(Number(percentFullValue), 1)} %`;
        }
        return 'N/A';
      },
    },
    ...
    },
    {
      ...
      accessor: (row: AssetSummaryModel) => {
        ...
        const displayValue = row.displayUOMParams?.reading;
        const scaledValue = row.scaledUOMParams?.reading;
        const percentFullValue = row.percentFullUOMParams?.reading;
    ...
    },
    {
      ...
      accessor: (row: AssetSummaryModel) => {
        if (row.readingTime) {
          return <FormatDateTime date={row.readingTime} />;
        }
        return '-';
      },
    },
    ...
    {
      id: 'assetShipto',
      apiProperty: 'AssetShipto',
      Header: t('ui.assetsummary.assetShipTo', 'Asset Ship To'),
      accessor: 'assetShipTo',
    },

    {
      id: 'dataChannelShipTo',
      apiProperty: 'DataChannelShipTo',
      Header: t('ui.assetsummary.datachannelShipTo', 'Data Channel Ship To'),
      accessor: 'dataChannelShipTo',
    },
  ...
  useEffect(() => {
    if (
      ...
      if (selectedAssetRecordIndex >= 0) {
        virtualizedListRef.current?.scrollToItem(
          selectedAssetRecordIndex,
          'center'
        );
      } else if (selectedDataChannelRecordIndex >= 0) {
        virtualizedListRef.current?.scrollToItem(
