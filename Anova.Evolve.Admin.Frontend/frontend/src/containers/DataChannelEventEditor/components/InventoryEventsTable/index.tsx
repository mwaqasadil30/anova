...
const InventoryEventsTable = ({
  ...
  const hasRosters = dataChannel?.inventoryEvents?.some(
    (event) => event?.rosters?.length
  );

  const areAllEventDelaysNull = dataChannel?.inventoryEvents?.every(
    (event) => event?.delay === null
  );

  const showSetpointColumn = dataChannel?.inventoryEvents?.some(
    (inventoryEvent) => inventoryEvent.isSetpointUpdateSupported
  );
        ...
        <CustomTable>
          ...
          <StyledTableBody>
            {dataChannel?.inventoryEvents?.map((inventoryEvent, eventIndex) => {
              const isEventDisabled = !inventoryEvent.isEnabled;
              ...
              // An alternative to formattedScaledUnitsText
                      ...
                      <Grid item xs={12}>
                        <StyledFieldGroup style={{ alignItems: 'flex-start' }}>
                          ...
                          <Field
                            ...
                            disabled={isDisabled || isEventDisabled}
                      ...
                      <Grid item xs>
                        <Grid container>
                          {inventoryEvent?.rosters?.length ? (
                            inventoryEvent?.rosters.split(',').map((roster) => {
                              return (
                                <Grid
                                  item
                                  xs="auto"
